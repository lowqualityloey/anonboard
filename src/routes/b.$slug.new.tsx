        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center justify-end space-x-3 pt-2">
            <Link
              to="/b/$slug"
              params={{ slug: board.slug }}
              className="rounded-md border border-border bg-surface px-4 py-2 text-xs font-medium text-text hover:border-border-hover transition-colors duration-fast"
            >
              Cancel
            </Link>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-xs font-semibold text-bg hover:bg-accent-hover transition-colors duration-fast disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
            >
              {isSubmitting && <Spinner className="h-3.5 w-3.5 animate-spin" />}
